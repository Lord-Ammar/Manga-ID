#Open Source Nih Ajg
#Kalo mau Ubash Kasi Credit Ajg
...
ses.headers["User-Agent"] = "Mozilla/5.0 (Linux; Android 10; Redmi 8A) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/95.0.4638.50 Mobile Safari/537.36"
result=[]
ch_result=[]
last_result=[]
def clear():
...
def baner():
	os.system("python logo1.py")
def process():
    for i in list("-\|/-\|"):
        print(f"\r{pr}Process\033[90m...{d}({r}{i}{d})",end="")
        sleep(0.1)
def cblg():
    lg=input(f"{pr}Cari Anime lagi? ({d}{c}y{d}/{c}n{p}) : {c}")
    if lg == "y" or lg == "Y":
        os.system("python main.py")
    elif lg == "n" or lg == "N":
        sys.exit(er+"THX FOR USING TOOLS")
        sleep(1)
        clear()
    else:
...
if __name__=="__main__":
     ...
     print("\033[90m----------------------------------------------\033[00m")
     print(f"{dn}Done.")
     print(f"{pr}Komik Saved Internal\n {c}-> {p}/sdcard/maid/{dir1}")
     cblg()
